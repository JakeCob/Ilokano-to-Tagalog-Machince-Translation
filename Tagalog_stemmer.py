...
def stemmer():
    ...
    print("1. NOT WORKING \n2. Input a FILE \n3. Output Results")
    options = input("Choose an option to execute: ")

    if options == "1":
        source = input("\nInput name of text file: (NOT WORKING) ")
        tokens = read_file(source)

    elif options == "2":
        source = input("\nInput FILE NAME ")
        tokens = singleline(source)
...
def validate(stemmed, errors):
    ...
    return format((float(check) / len(stemmed) * 100), '.2f')


def singleline(source):
